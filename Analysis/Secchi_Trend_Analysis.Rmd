...
Accordingly, we chose to remove those values from the Sebago Lake record.
```{r}
secchi_data <- secchi_data %>%
...
Note that the default test for statistical significance used by `mblm()` fails regularly with the short data records over the last ten years.
...
than the linear model slope, but the regression is not significantly different
from a 1:1 line.  The two lakes, `MIDAS == 3420` and `MIDAS = 3448` show
statistically significant improving trends by linear model, but not by Theil-Sen estimators.
...
Note that before we removed Station 50 from the Sebago Lake (MIDAS == 5786)
data, has a strong negative slope by least squares, but a (small) positive slope
by Thiel-Sen. After removing the Station 50 points, results are more
consistent.
...
```
Relevant sample sizes and starting dates for Duchk andBog ponds are as follow3s:
