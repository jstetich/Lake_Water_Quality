...
# Problems at Sebago Lake Results
```{r}
secchi_data %>%
  filter(MIDAS == 5786) %>%
...
We could also run these models using the nested dataframes approach used
for loking at metals in out Toxics data, but this "for loop" aproach works
...
plt <-  ggplot(lt_results, aes(Slope, TSSlope))  +
  ...
  geom_text(aes(label = MIDAS), size = 3) + 

  geom_smooth(method = 'lm', se = FALSE)
plt
```
So results are similar either way.  The TS Slope tends to be slightly lower
than the linear model slope.  The two lakes, `MIDAS == 3420` and
`MIDAS = 3448` show statistically significant improving trends by linear model,
but not by Theil-Sen estimators.
...
Note that Sebago Lake (MIDAS == 5786) has a strong negative slope by
least squares, but a (small) positive slope by Thiel-Sen. It is also an outlier 
...
st_results %>%
  group_by(lm_category)%>%
  summarize(n=n())

```

### Results if Thiel-Sen USed
We test significance by Kendall's Tau.
```{r}
st_results <- st_results %>%
...
lt_results_minimal <- lt_results %>%
  select(MIDAS, Samples, Stations, Years, Median, Lake, TSSlope, TS_category)
```
...
combined_results <- lt_results_minimal %>%
  full_join(st_results_minimal,
            by = c("MIDAS", "Samples", "Stations", "Lake"),
            suffix = c('_lt', '_st'))
```
...
```
So if there is a relationship, it's predominately due to two lakes, so we won't
take it too seriously.


# Could we generate a graphic showing the trndlines for all lakes?
Probably only by refitting models...
```{r}
secchi_data %>%
filter(MIDAS %in% TrendLakesMI)
ggplot()

}


ggplot(lt_predss, aes(year))
