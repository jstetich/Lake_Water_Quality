...
the the most abundant TSI value is based oon Secchi Depth, and is a relatively
...
indexes have problems that took us some time to unravel.  We present definitions
...
We were able to duplicate values for published data using this formula, so we
consider it correct.
...
comparison purposes, and examining their form may help determine how
the Maine formulae have gone awry.
...
# Taylor Pond Example
There appears to be some inconsistency in Maine over TSI formulae. For example,
the [Taylor Pond Association website](http://taylorpond.org/) provides the
Carlson version of the formulae (as rewritten by NALMA) for calculating TSI
based on Secchi depth and Total Phosphorus.
...
tsi_data %>%
  ...
  arrange(Tsi_Sec)
```
So, Tsi_Sec is by far the most abundant estimate of TSI. Chlorophyll-based 
and phosphorus-based TSI values are somewhat less abundant, but still fairly
...
plt
```
 So Tsi_Sec and Tsi_Chl are fairly well correlated, with a slope close to 1:1.  Tsi_Tpec appears to be almost as well correlated with the other
 indexes, but 
...
## Grouped Graphic
We can show  several TSI variants on one graph, but it is slightly
hard to read.
```{r}
tsi_long <- tsi_data %>%
  select(-Tsi_Tppg) %>%
...
drivers change.  The two formulae give similar values for 
* $ SD \approx 7$
* $ Chl~a~ \approx 3.5$
* $ TP \approx 6$

##### Secchi Depth TSI
We reverse the X axis so this plot to visually aligns with the other two, with
